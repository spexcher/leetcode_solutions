...
//  Created      : 15.07.2026 at 04:11:08
...
public:
    ll dp[20][2][1024][2][2];
    ll ans(int idx, bool tight, int mask, bool lz, bool good, string n) {
        // cout << "wkjfw";
        if (idx == sz(n)) {
            if (good)
                return 1LL;
        ...
        if (dp[idx][tight][mask][lz][good] != -1LL)
            return dp[idx][tight][mask][lz][good];
        ll res = 0LL;
        ll ub = (tight) ? (n[idx] - '0') : 9LL;
        ff(dig, 0LL, ub) {
            if (lz and dig == 0LL)
                res += ans(idx + 1, (tight and dig == ub), 0, 1, 0, n);

            else {
                res += ans(idx + 1, (tight and dig == ub), mask | (1 << dig),
                           (lz and dig == 0LL), good or ((mask>>dig) & 1) , n);
            }
        }
        return dp[idx][tight][mask][lz][good] = res;
    }

    int numDupDigitsAtMostN(int n) {
        memset(dp, -1, sizeof(dp));
        return ans(0, 1, 0, 1, 0, to_string(n));
    }
};