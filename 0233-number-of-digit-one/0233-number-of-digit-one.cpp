...
//  Created      : 15.07.2026 at 03:35:09
...
public:
    ll dp[20][2][11][2];
    ll ans(int idx, bool tight, int c1, bool lz, string n) {
        if (idx == sz(n))
            return c1;
        if (dp[idx][tight][c1][lz] != -1LL)
            return dp[idx][tight][c1][lz];
        ll res = 0LL;
        ll ub = (tight) ? (n[idx] - '0') : 9LL;
        ff(dig, 0LL, ub) {
            res += ans(idx + 1, (tight and dig == (n[idx] - '0')),
                       c1 + (dig == 1), (lz and dig == 0LL), n);
        }
        return dp[idx][tight][c1][lz] = res;
    }

    int countDigitOne(int n) {
        memset(dp, -1, sizeof(dp));
        return ans(0, 1, 0, 0, to_string(n));
    }
};