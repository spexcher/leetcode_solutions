...
//  Created      : 09.07.2026 at 15:21:16
...
class Solution {
public:
    vector<bool> pathExistenceQueries(int n, vector<int>& v, int md, vector<vector<int>>& qu) {
        mll par;
        par[0]=0;
        ff(i,1,n-1){
            if(abs(v[i]-v[i-1])<=md)par[i]=par[i-1];
            else par[i]=par[i-1]+1;
        }
        vb res;
        for(auto x:qu){
            if(par[x[0]]==par[x[1]])res.pb(true);
            else res.pb(false);
        }
        return res;
    }
